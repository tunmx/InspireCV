#ifndef INSPIRECV_BACKENDS_OKCV_ADAPTER_TRANSFORM_MATRIX_BACKEND_H_
#define INSPIRECV_BACKENDS_OKCV_ADAPTER_TRANSFORM_MATRIX_BACKEND_H_
...
public:
    ...
    void Translate(float dx, float dy) {
        Prepend({1.0f, 0.0f, dx, 0.0f, 1.0f, dy});
    }

    void Scale(float sx, float sy) {
        Prepend({sx, 0.0f, 0.0f, 0.0f, sy, 0.0f});
    }
...
private:
    void Prepend(const geometry::Affine2D &outer) {
        const geometry::Affine2D inner = {
          matrix_[0], matrix_[1], matrix_[2], matrix_[3], matrix_[4], matrix_[5]};
        const auto composed = geometry::Compose(outer, inner);
        matrix_ = okcv::TransformMatrix({composed.xx, composed.xy, composed.x_offset,
                                         composed.yx, composed.yy, composed.y_offset});
    }
...
#endif  // INSPIRECV_BACKENDS_OKCV_ADAPTER_TRANSFORM_MATRIX_BACKEND_H_
