#ifndef INSPIRECV_BACKENDS_OPENCV_ADAPTER_IMAGE_BACKEND_H_
#define INSPIRECV_BACKENDS_OPENCV_ADAPTER_IMAGE_BACKEND_H_
...
public:
    ...
    Impl(const cv::Mat& mat) : image_(mat) {}
    ...
    Impl(int width, int height, int channels, const Pixel* data = nullptr,
        ...
        if (data && !copy_data) {
            image_ = cv::Mat(height, width, type, const_cast<Pixel*>(data));
            is_external_ = true;
            return;
        }
    ...
    void Reset(int width, int height, int channels, const Pixel* data = nullptr,
        ...
        if (is_external_) image_.release();
        image_.create(height, width, type);
        is_external_ = false;
        if (data) {
            std::memcpy(image_.data, data, sizeof(Pixel) * width * height * channels);
    ...
    int Channels() const {
        return image_.empty() ? 0 : image_.channels();
    }
    bool Empty() const {
        return image_.empty();
    }
    ...
    ImageT<Pixel> Add(double value) const {
        ...
        if (std::is_same<Pixel, uint8_t>::value) {
            // Wrap-around semantics to match OKCV behavior and tests
            result.impl_->image_.create(image_.rows, image_.cols, image_.type());
            const int rows = image_.rows;
            const int cols_times_c = image_.cols * image_.channels();
            for (int y = 0; y < rows; ++y) {
                const uint8_t* src = image_.ptr<uint8_t>(y);
                uint8_t* dst = result.impl_->image_.template ptr<uint8_t>(y);
                for (int x = 0; x < cols_times_c; ++x) {
                    float v = static_cast<float>(src[x]) + static_cast<float>(value);
                    dst[x] = static_cast<uint8_t>(static_cast<int>(v));
                }
            }
            return result;
        } else {
            // Float: linear add
            cv::add(image_, cv::Scalar::all(value), result.impl_->image_, cv::noArray(), CV_32F);
            return result;
        }
    }
    ...
    ImageT<Pixel> Pad(int top, int bottom, int left, int right, const std::vector<double>& color) const {
        ...
        if (color.empty()) {
            ...
            // Interpret color in native channel order (B, G, R)
            // If only one value provided, broadcast to all channels
            if (color.size() == 1) {
                cv_color = cv::Scalar(color[0], color[0], color[0]);
            } else {
                double b = color.size() > 0 ? color[0] : 0;
                double g = color.size() > 1 ? color[1] : b;
                double r = color.size() > 2 ? color[2] : g;
                cv_color = cv::Scalar(b, g, r);
            }
        } else if (image_.channels() == 4) {
            // Interpret color in native channel order (B, G, R, A)
            // If only one value provided, broadcast to all channels
            if (color.size() == 1) {
                cv_color = cv::Scalar(color[0], color[0], color[0], color[0]);
            } else {
                double b = color.size() > 0 ? color[0] : 0;
                double g = color.size() > 1 ? color[1] : b;
                double r = color.size() > 2 ? color[2] : g;
                double a = color.size() > 3 ? color[3] : r;
                cv_color = cv::Scalar(b, g, r, a);
            }
        }
    ...
    ImageT<Pixel> ToGray() const {
        ...
        // Backend channel order is BGR; prefer OpenCV intrinsic implementation
    ...
    void DrawLine(const Point<int>& p1, const Point<int>& p2, const std::vector<double>& color,
        ...
        if (!color.empty()) {
            if (image_.channels() >= 3) {
                // Interpret color in native order (B, G, R[, A])
                double b = color[0];
                double g = color.size() > 1 ? color[1] : b;
                double r = color.size() > 2 ? color[2] : g;
                cv_color = cv::Scalar(b, g, r);
    ...
    ImageT<Pixel> Threshold(double thresh, double maxval, int type) const {
        ImageT<Pixel> result;
        cv::Mat result_mat;
        // Use OpenCV intrinsic; semantics: THRESH_BINARY uses '>' not '>='
    ...
    ImageT<Pixel> MeanChannels() const {
        ...
        ImageT<Pixel> result;
        const int rows = image_.rows;
        const int cols = image_.cols;
        const int cn = image_.channels();
        if (std::is_same<Pixel, uint8_t>::value) {
            // Deterministic integer mean using exact integer divide (matches tests)
            result.impl_->image_.create(rows, cols, CV_8UC1);
            for (int y = 0; y < rows; ++y) {
                const uint8_t* src = image_.ptr<uint8_t>(y);
                uint8_t* dst = result.impl_->image_.template ptr<uint8_t>(y);
                for (int x = 0; x < cols; ++x) {
                    const int base = x * cn;
                    unsigned int sum = 0;
                    for (int c = 0; c < cn; ++c) sum += static_cast<unsigned int>(src[base + c]);
                    dst[x] = static_cast<uint8_t>(sum / static_cast<unsigned int>(cn));
                }
            }
            return result;
        } else {
            // Float: true mean
            result.impl_->image_.create(rows, cols, CV_32FC1);
            for (int y = 0; y < rows; ++y) {
                const float* src = image_.ptr<float>(y);
                float* dst = result.impl_->image_.template ptr<float>(y);
                for (int x = 0; x < cols; ++x) {
                    const int base = x * cn;
                    float sum = 0.0f;
                    for (int c = 0; c < cn; ++c) sum += src[base + c];
                    dst[x] = sum / static_cast<float>(cn);
                }
            }
            return result;
        }
...
#endif  // INSPIRECV_BACKENDS_OPENCV_ADAPTER_IMAGE_BACKEND_H_
