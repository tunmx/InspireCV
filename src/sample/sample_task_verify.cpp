...
static void ensureDir(const std::string& dir) {
    ...
    for (size_t i = 0; i < dir.size(); ++i) {
        ...
        path.push_back(c);
        if (c == '/') {
            if (path.size() == 1 && path[0] == '/') continue;  // root
            if (!exists(path)) {
...
static void saveBinary(const std::string& path, const void* data, size_t size) {
...
static void testGeometry(const cv::Mat& bgr) {
    ...
    for (auto& kv : mats) {
        ...
        if (name == "identity") {
            cv::imwrite(gOutDir + "/gt/SRC_BGR.png", bgr);
        }
        cv::imwrite(gOutDir + "/gt/GT_geo_" + base + ".png", cvOut);
...
int main(int argc, char** argv) {
    ...
    ensureDir(gOutDir + "/gt");
    {
        ...
        cv::imwrite(gOutDir + "/gt/GT_bgr2rgb.png", cvRef);
    }
    {
        ...
        saveBinary(gOutDir + "/gt/SRC_nv21_w" + std::to_string(bgr.cols) + "_h" + std::to_string(bgr.rows) + ".bin",
                   nv21.data, static_cast<size_t>(nv21.total()));
        cv::imwrite(gOutDir + "/gt/GT_nv21_to_bgr.png", cvRef);
    }
    {
        ...
        saveBinary(gOutDir + "/gt/SRC_nv12_w" + std::to_string(bgr.cols) + "_h" + std::to_string(bgr.rows) + ".bin",
                   nv12.data, static_cast<size_t>(nv12.total()));
        cv::imwrite(gOutDir + "/gt/GT_nv12_to_bgr.png", cvRef);
    }
    {
        ...
        saveBinary(gOutDir + "/gt/SRC_i420_w" + std::to_string(bgr.cols) + "_h" + std::to_string(bgr.rows) + ".bin",
                   i420.data, static_cast<size_t>(i420.total()));
        cv::imwrite(gOutDir + "/gt/GT_i420_to_bgr.png", cvRef);
    }
